...
public class PtyProcess extends Process {
  ...
  protected PtyProcess(String[] cmdarray, String[] envp, File dir, Pty pty) throws IOException {
    String dirpath = ".";
    if (dir != null) {
...
//        out = new SpawnerOutputStream(fChannels[0]);
  ...
  public synchronized InputStream getErrorStream() {
    if (null == err) {
      if (myPty != null && !myPty.isMyConsole()) {
...
//    }
  ...
  private void execInPty(String[] command, String[] environment, String workingDirectory, Pty pty) throws IOException {
    ...
    final boolean console = pty.isMyConsole();
    // int fdm = pty.get
  ...
  class Reaper extends Thread {
    ...
    public void run() {
      ...
      synchronized (PtyProcess.this) {
        PtyProcess.this.notifyAll();
      }
      if (pid != -1) {
