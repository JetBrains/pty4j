...
class ExtractedNative {
  ...
  private void init() {
    try {
      myResourceOsArchSubPath = PtyUtil.requireNonNullElse(myResourceOsArchSubPath, PtyUtil.getNativeLibraryOsArchSubPath());
      myResourceNamePrefix = PtyUtil.requireNonNullElse(myResourceNamePrefix, DEFAULT_RESOURCE_NAME_PREFIX);
      synchronized (this) {
        if (!myInitialized) {
          doInit();
        }
